...
int64 stream3[print_num];
int64 stream4[print_num];
int64 t1, t2;
int64 real_cycle;
...
*/
int output_csv(int length)
{
	FILE *fp;
	printf("Writing file %d\n", nbOfFilesOut);
	int i;
	char buffer[50];
	sprintf(buffer, ".\\Outputs\\testOut%d.csv", nbOfFilesOut);
	fp = fopen(buffer, "w");
	if (fp == NULL)
		return 0;
	for (i = 0; i < length; i++)
	{
		fprintf(fp, "%d; %lld; %lld; %lld; %lld\n", i, stream1[i], stream2[i], stream3[i], stream4[i]);
	}
	fclose(fp);
	//nbOfFilesOut++;
	return 1;
		...
		if (ec_config_init(FALSE) > 0)
		{
			...
			if (ec_slave[0].state == EC_STATE_OPERATIONAL)
			{
				...
				while (1)
				{
					...
					printf("Processdata cycle %5d , Wck %2d, DCtime %12lld, ct %9lld, O:",
						   dorun, globalWkc, ec_DCtime, real_cycle);
